...
$stage = Join-Path $repoRoot 'tmp\flex-validation'
...
$testProject = '<Project xmlns="http://schemas.microsoft.com/developer/msbuild/2003"><PropertyGroup><MainSource>../../tools/test-flex-preview.dpr</MainSource><AppType>Console</AppType><FrameworkType>VCL</FrameworkType><DCC_Namespace>Winapi;System;System.Win;Vcl;Vcl.Imaging;Vcl.Touch;Vcl.Samples;Vcl.Shell;Data;Data.Win;Datasnap;Xml;Xml.Win;Web;Web.Win;Soap;Soap.Win</DCC_Namespace><DCC_UnitSearchPath>../../sources;$(DCC_UnitSearchPath)</DCC_UnitSearchPath><DCC_ExeOutput>.</DCC_ExeOutput><DCC_DcuOutput>test-dcu</DCC_DcuOutput></PropertyGroup><ItemGroup><DelphiCompile Include="$(MainSource)"><MainSource>MainSource</MainSource></DelphiCompile></ItemGroup><Import Project="$(BDS)\Bin\CodeGear.Delphi.Targets"/></Project>'
[IO.File]::WriteAllText((Join-Path $stage 'test-flex-preview.dproj'), $testProject)
& $msbuild (Join-Path $stage 'test-flex-preview.dproj') @common
if ($LASTEXITCODE -ne 0) { throw 'Flex preview test build failed.' }
& (Join-Path $stage 'test-flex-preview.exe')
if ($LASTEXITCODE -ne 0) { throw 'Flex preview test failed.' }
